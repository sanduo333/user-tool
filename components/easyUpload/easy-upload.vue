		...
		<view class="upload">
			<block v-for="(upload,index) in uploads" :key="index">
				<view class="uplode-file">
					<image v-if="types == 'image'" class="uploade-img" :src="upload" :data-src="upload"
					...
					<video v-if="types == 'video'" class="uploade-img" :src="upload" controls>
						<cover-image v-if="types == 'video'" class="clear-one-icon" :src="clearIcon"
							@tap="delImage(index)"></cover-image>
					</video>
				</view>
			</block>
	...
	export default {
		...
		methods: {
			...
			delImage(index) {
				...
				if (this.uploads[index].substring(0, 4) !== 'http' || this.uploads[index].substring(0, 11) ==
					'http://tmp/') {
			...
			uploadFile(path) {
				uni.getFileSystemManager().readFile({
					...
					encoding: 'base64',
					success: (res) => {
						requestManger.post('/rights/v1/images_b64', {
							fileName: "test.jpg",
							b64: res.data
						}).then(res => {
							this.$emit('successImage', res)
							this.$emit('imgCount', this.uploads);
						}).catch(err => {})
					},
					fail: (err) => {}
